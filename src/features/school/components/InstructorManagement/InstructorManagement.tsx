              ...
              <div className="flex flex-col gap-2">
                ...
                {!isEdit ? (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium ${(instructors.find(i => i.id === formData.id) as any)?.emailVerified
                        ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800'
                        : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800'
                      }`}>
                      <span>{(instructors.find(i => i.id === formData.id) as any)?.emailVerified ? '✅ E-posta doğrulandı' : '⚠️ E-posta henüz doğrulanmadı'}</span>
                    </div>
                    {/* Şifre Sıfırlama */}
                    ...
                    {!(instructors.find(i => i.id === formData.id) as any)?.emailVerified && (
